...
const char TAG_AP[13] = "AccessPoint";
...
{
    server.on("/", HTTP_GET, [configuration]()
              { 
    ...
    l_debug(TAG_AP, "AP Webserver active");
    server.begin();
}

void ap_setup(const Configuration &configuration)
{
    WiFi.mode(WIFI_AP);
    WiFi.softAP(AP_SSID);
    dnsServer.setErrorReplyCode(DNSReplyCode::NoError);
    dnsServer.start(AP_DNS_PORT, "*", WiFi.softAPIP());
    ap_webserver(configuration);
}
